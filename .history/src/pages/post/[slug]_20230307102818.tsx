...
const qtdPalvras = post.data.content.reduce((acc,item)=>{
  const heading = item.heading.replace(/\s/g, '').trim().split('').length
  const body = item.body.map(bo =>{
    return bo.text.replace(/\s/g, '').trim().split('').length
  })
...
console.log(qtdPalvras)




// const test = he.reduce((acc,item)=>{
//   return acc+=item.replace(/\s/g, '').trim().split('').length
// },0)
// console.log(test)
// const qtdPalavras = post.data.content.reduce((result,item) => {
